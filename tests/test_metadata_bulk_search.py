...
class MetadataBulkSearchTestCase(unittest.IsolatedAsyncioTestCase):
    def setUp(self) -> None:
        ...
        self.app.workers = SimpleNamespace(run=AsyncMock(side_effect=run))
...
class RunMetadataBulkSearchTests(MetadataBulkSearchTestCase):
    ...
    async def test_high_confidence_match_is_auto_applied(self) -> None:
        ...
        self.app.service.search_metadata = Mock(return_value=[candidate])
    ...
    async def test_no_candidates_is_flagged_as_not_found(self) -> None:
        track = _track(1)
        self.app.service.search_metadata = Mock(return_value=[])
        task_id = self._start([track])
    ...
    async def test_per_track_error_does_not_abort_the_batch(self) -> None:
        ...
        def search(track_id, provider_ids, limit):
            if track_id == 1:
                raise MetadataProviderError("boom")
            return [good_candidate]

        self.app.service.search_metadata = Mock(side_effect=search)
        self.app.service.apply_candidate = Mock()
    ...
    async def test_cancel_before_processing_marks_task_cancelled(self) -> None:
        ...
        self.assertNotIn(task_id, self.app._metadata_bulk_task_contexts)
        self.app.service.search_metadata.assert_not_called()

    async def test_pause_before_processing_keeps_context_for_resume(self) -> None:
